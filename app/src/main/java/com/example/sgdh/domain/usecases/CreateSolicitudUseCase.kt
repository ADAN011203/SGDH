...
import com.example.sgdh.data.models.solicitudes.DetalleSolicitud
...
class CreateSolicitudUseCase @Inject constructor(
    private val solicitudRepository: SolicitudRepository
) {
    operator fun invoke(
        justificacion: String,
        detalles: List<DetalleSolicitud>
    ): Flow<Resource<Solicitud>> = flow {
        emit(Resource.Loading())
        try {
            val result = solicitudRepository.createSolicitud()
            emit(result)
        } catch (e: Exception) {
